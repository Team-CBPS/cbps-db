...
function submit_scope()
{
	...
	{
		scope_entry = document.getElementById("scope_name").value = "";
		document.getElementById("scope_status").innerText = "No config entries!";
		form_plugin_scope.value = "None";
		return;
	}
	else
...
function remove_dependancy()
{
	if(DEPENDS_LIST.length >= 1)
	{
		var depends_dropdown = document.getElementById("depends_options");
		DEPENDS_LIST.pop(depends_dropdown.selectedIndex);
		depends_dropdown.selectedOptions[0].remove();;
		submit_scope();
	}
	else
	{
		document.getElementById("depends_status").innerText = "Nothing to remove!";
		return;
	}

}
function add_dependancy()
{
	var depends_dropdown = document.getElementById("depends_options");
	var depends_entry = document.getElementById("depends_id").value;
	xhr = new XMLHttpRequest();
	xhr.open("GET","/check_exists.php?id="+depends_entry,false);
	xhr.send();
	var exists = xhr.responseText;
	if(exists == "True")
	{
		var opt = document.createElement('option');
		opt.value = depends_entry;
		opt.innerText = depends_entry;
		depends_dropdown.add(opt);
		DEPENDS_LIST.push(depends_entry);
		depends_entry = document.getElementById("depends_id").value = "";
		submit_dependancy();
	}
	else
	{
		document.getElementById("depends_status").innerText = "Dependency not found in CbpsDB.";
	}
}
function submit_dependancy()
{
	if(DEPENDS_LIST.length == 0)
	{
		depends_entry = document.getElementById("depends_id").value = "";
		document.getElementById("depends_status").innerText = "No dependancys!";
		form_dependancy_list.value = "None";
		return;
	}
	else
	{
		document.getElementById("depends_status").innerText = "Dependancy list updated.";
		form_dependancy_list.value = DEPENDS_LIST.join("|");
	}
}
...
function edit(what_to_edit)
{
	switch(what_to_edit)
	{
		...
		case 'download_src':
			...
			break;
		case 'dependancy_list':
			show_step(step_enter_depends);
			break;
...
{
	document.getElementById("submit_icon0_preview").src = "/img/app_default.png";
	form_download_icon0.value = "None";
}
...
function submit_icon0()
{
	...
	if(icon0_url.indexOf("/img/app_default.png") != -1)
	{
		form_download_icon0.value = "None";
		return;
	}
...
window.addEventListener('load', function () {
	...
	form_dependancy_list = document.getElementById("depends_on");
	...
	step_enter_depends = document.getElementById("enter_depends");
	step_enter_scope = document.getElementById("enter_scope");
	step_please_verify = document.getElementById("please_verify");


	steps = [step_enter_git,step_enter_file,step_enter_title,step_enter_readme,step_enter_credits,step_please_verify,step_enter_depends]
...
if(isset($_GET['id'])) 
{
	...
	echo('<select id="id_select" style="display: none;" class="submit-dropdown" onchange="select_update()">');
		...
		<option value="dependancy_list">Dependancies</option>
		');
	...
	<div id="please_verify" class="submit-form" >
		<div class="submit-inputs">
			...
			<form action="/git/edit_entry.php" method="post" id="mainform" target="_blank">
				...
				<input type="text" id="depends_on" name="depends_on" value="'.htmlspecialchars($csv[DEPENDS],ENT_QUOTES).'" hidden="true">
	...
	echo('
		...
		<div id="enter_depends" class="submit-form" >
			<div class="submit-inputs">

			  <select id="depends_options" class="submit-dropdown" value="None">
					');
						if(strcmp($csv[DEPENDS],"None") !== 0)
						{
							$depends = get_list($csv[DEPENDS]);
							for($i = 0; $i < sizeof($depends); $i++)
							{
								echo('<option value="'.htmlspecialchars($depends[$i],ENT_QUOTES).'">'.htmlspecialchars($depends[$i],ENT_QUOTES)."</option>\n");
							}
							
							echo('<script> DEPENDS_LIST = [');
							for($i = 0; $i < sizeof($depends); $i++)
							{
								echo('"'.htmlspecialchars($depends[$i],ENT_QUOTES).'"');
								if($i+1 < sizeof($depends))
								{
									echo(",");
								}
							}
						...
						{
							echo('<script> DEPENDS_LIST = []; </script>');
						}
					echo('
			  </select><br><br>


			  <textarea cols="80" rows="1" id="depends_id" class="submit-textarea"></textarea>
			  <button onclick="add_dependancy()" id="dependancy_add" class="submit-button"><img width="15" height="15" src="/img/add_icon.png"></img></button>
			  <button onclick="remove_dependancy()" id="dependancy_remove" class="submit-button"><img width="15" height="15" src="/img/remove_icon.png"></img></button><br><br>
			</div>



			<div id="depends_status" class="submit-status">
				Waiting for input...
			</div><br>
		</div>
