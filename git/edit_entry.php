...
if(isset($_POST["g-recaptcha-response"]))
{
	...
	$data = array(
		'secret' => 'YOUR_RECAPTCHA_BACKEND_KEY_HERE',
...
remove_entry("cbps-db/cbpsdb.csv",$id);

$csvdata = file_get_contents("cbps-db/cbpsdb.csv");
$csvdata .= $id.",".$title.",".$credits.",".$download_icon0.",None,".$download_url.",None,".$download_readme.",None,".$download_src.",None,".(string)(time()).",".$config_type.",".$scope.",".$type.",".$depends_on.",True\n";
file_put_contents("cbps-db/cbpsdb.csv",$csvdata);
sort_newest_csv("cbps-db/cbpsdb.csv");
exec("sh push_latest.sh \"".$branch_no."\"");
header("Location: ".exec("python3 create_pr.py ".escapeshellarg($branch_no)." ".escapeshellarg('Edit '.$title)." ".escapeshellarg($prepend." ".$id."\t".$title."   ".$credits."   ".$download_icon0."   None   ".$download_url."   None   ".$download_readme."   None   ".$download_src."   None   ".(string)(time())."   ".$config_type."   ".$scope."   ".$type."   ".$depends_on."   True")));
