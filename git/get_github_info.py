...
def getReadme(github, branch):
	try:
		req = requests.get(github)
		htmlTree = html.fromstring(req.text)
		rdme = htmlTree.xpath("//*[@id=\"readme\"]/div[1]/h2")
		rdme = rdme[0].text.replace("\n","").replace(" ","")
...
if checkIsGithub(github_url):
	...
	if not github_url.endswith(".git"):
		github_url += ".git"
	dict = {"credits":repoOwner,"name":repoName,"latest_releases":releases,	"readme_md":readme, "icons":icon0,"git_uri":github_url}
